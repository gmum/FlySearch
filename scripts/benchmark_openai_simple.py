...
subset = "hr_bench"
dummy_permutation_shift = subset == "hr_bench"

RUN_NAME = f"{subset}_test_permutation_shift_naive_FULL_B"
...
def main():
    if subset == "hr_bench":
        ...
        ds = VstarSubBenchDataset(f"/home/dominik/vstar_bench/{subset}",
                                  transform=pil_to_opencv)
    ...
    for i, (image, question, options, answer) in bar:
        psg = PermutationShiftGenerator(options, dummy=dummy_permutation_shift)
